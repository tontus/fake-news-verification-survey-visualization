...
function EducationVerificationRadarChart({ csvData, width = 700, height = 400, isMobile = false }) {
    const svgRef = useRef()
    ...
    const processData = useCallback((rawData) => {
        ...
        // Group by education qualifications and calculate averages
        ...
        cleanData.forEach(row => {
            ...
            educationGroups[education].verification_importance.push(parseFloat(row.verification_importance))
            educationGroups[education].trustworthiness.push(parseFloat(row.trustworthiness))
            educationGroups[education].verification_level.push(parseFloat(row.verification_level))
        })

        console.log('Education groups:', educationGroups)

        // Calculate averages for each education level
        const radarData = Object.keys(educationGroups).map(education => {
            const group = educationGroups[education]
    ...
    useEffect(() => {
        if (csvData) {
            console.log('Processing CSV data in EducationVerificationRadarChart:', csvData)
            processData(csvData)
            setLoading(false)
        }
    }, [csvData, processData])
    ...
    useEffect(() => {
        ...
        const createRadarChart = () => {
            ...
            const minValue = 4 // Start from 4 as there are no values below this
            const rScale = d3.scaleLinear()
                .range([0, radius])
                .domain([minValue, maxValue])

            // Create color scale for different education levels
            const colorScale = d3.scaleOrdinal()
                .domain(data.radarData.map(d => d.education))
                .range(['#1d2932ff', '#e74c3c', '#f39c12', '#2ecc71', '#9b59b6', '#e67e22', '#34495e', '#3498db'])
            ...
            // Draw radar areas for each education level
            ...
            data.radarData.forEach((group) => {
                ...
                areasGroup.append("path")
                    .datum(group.metrics)
                    .attr("class", `radar-area-${group.education.replace(/\s+/g, '-').toLowerCase()}`)
                    .attr("d", radarArea)
                    .style("fill", colorScale(group.education))
                    .style("fill-opacity", 0.2)
                    .style("stroke", colorScale(group.education))
                    .style("stroke-width", "2px")

                // Draw the outline in the areas group
                areasGroup.append("path")
                    .datum(group.metrics)
                    .attr("class", `radar-line-${group.education.replace(/\s+/g, '-').toLowerCase()}`)
                    .attr("d", radarLine)
                    .style("fill", "none")
                    .style("stroke", colorScale(group.education))
            ...
            data.radarData.forEach((group) => {
                group.metrics.forEach((metric, i) => {
                    ...
                    pointsGroup.append("circle")
                        .attr("class", `radar-point-${group.education.replace(/\s+/g, '-').toLowerCase()}`)
                        .attr("cx", x)
                        .attr("cy", y)
            ...
            function highlightGroup(education) {
                const classNameSafe = education.replace(/\s+/g, '-').toLowerCase()
            ...
            data.radarData.forEach((group, i) => {
                const legendItem = legend.append("g")
                    ...
                    .style("cursor", "pointer")
                    .on("mouseover", function (event) {
                        highlightGroup(group.education)
                ...
                legendItem.append("text")
                    ...
                    .style("fill", "#333")
                    .text(`${group.education} (n=${group.count})`)
            })
            ...
            function showTooltip(event, group, metric) {
                ...
                const tooltipText = `${group.education}\n${metric.axis}: ${metric.value.toFixed(2)}\nSample size: ${group.count}`
            ...
            svg.append("text")
                ...
                .style("font-weight", "bold")
                .style("fill", "#333")
                .text("Verification Metrics by Education Level")
        ...
        return (
            ...
            >
                ...
                <Typography variant="h6" color="text.secondary">
                    Loading education verification data...
                </Typography>
            </Box>
        )
    }
...
export default EducationVerificationRadarChart
