...
function SocialMediaBoxPlotChart({ csvData, width = 700, height = 400, isMobile = false }) {
    const svgRef = useRef()
    ...
    const processData = useCallback((rawData) => {
        ...
        const cleanData = rawData.filter(row =>
            ...
            row.post_per_day && row.post_per_day.trim() !== '' &&
            !isNaN(parseFloat(row.minute_per_day)) &&
            !isNaN(parseFloat(row.post_per_day))
        )
        ...
        const processedData = cleanData.map(row => ({
            ...
            postsPerDay: parseFloat(row.post_per_day)
        }))
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if (groupData.length > 0) {
                const postsData = groupData.map(d => d.postsPerDay).sort((a, b) => a - b)

                // Calculate quartiles
                const q1 = d3.quantile(postsData, 0.25)
                const median = d3.quantile(postsData, 0.5)
                const q3 = d3.quantile(postsData, 0.75)
                const min = d3.min(postsData)
                const max = d3.max(postsData)
                ...
                const outliers = postsData.filter(d => d < lowerFence || d > upperFence)
                const nonOutliers = postsData.filter(d => d >= lowerFence && d <= upperFence)
    ...
    useEffect(() => {
        if (csvData) {
            console.log('Processing CSV data in SocialMediaBoxPlotChart:', csvData)
            processData(csvData)
            setLoading(false)
        }
    }, [csvData, processData])
    ...
    useEffect(() => {
        ...
        const createBoxPlot = () => {
            ...
            svg.append("text")
                ...
                .style("font-weight", "bold")
                .style("fill", "#333")
                .text("Posts per Day by Social Media Usage")
        ...
        return (
            ...
            >
                ...
                <Typography variant="h6" color="text.secondary">
                    Loading social media usage data...
                </Typography>
            </Box>
        )
    }
...
export default SocialMediaBoxPlotChart
